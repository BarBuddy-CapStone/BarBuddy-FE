...
function CustomerBookingHistory() {
    ...
    useEffect(() => {
        const fetchBookings = async (pageIndex = 1) => {
            try {
                const response = await BookingService.getAllBookings(accountId, null, pageIndex, 5); // Fetch bookings with pagination
    ...
    return (
        <section className={`flex flex-col px-10 py-8 mx-auto w-full max-w-6xl rounded-md bg-neutral-800 shadow-md max-md:px-5 max-md:mt-10 ${bookings.length > 1 ? 'min-h-[500px]' : 'min-h-[300px]'}`}>
    <div className="flex justify-between items-center mb-8 relative">
        ...
        <h2 className="text-3xl text-amber-400 text-center w-full">Lịch sử đặt bàn</h2>
    </div>
    ...
    <div className="space-y-6 flex-1">
        {bookings.map((booking, index) => (
            <React.Fragment key={booking.bookingId}>
                ...
                <div className="flex items-start justify-between cursor-pointer" onClick={() => navigate(`/booking-detail/${booking.bookingId}`)}>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
        <div className="bg-neutral-800 p-8 w-96 rounded-md shadow-2xl border border-neutral-800">
            <h2 className="text-xl mb-6 text-amber-400 text-center">Xác nhận hủy đặt bàn?</h2>
            <div className="flex justify-between space-x-4"> {/* Added space-x-4 for spacing between buttons */}
                <button
                ...
                <button
                    ...
                    onClick={() => handleCancelBooking(selectedBookingId)} // Xác nhận button on the right
                >
                    Xác nhận
                </button>
            </div>
