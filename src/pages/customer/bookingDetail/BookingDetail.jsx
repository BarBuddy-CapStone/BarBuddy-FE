...
function BookingDetailPage() {
  ...
  return (
    <div className={`flex flex-col px-8 ${bookingDrinksList && bookingDrinksList.length > 0 ? 'lg:flex-row gap-8' : 'justify-center items-center'}`}>
      {/* Main Content */}
      <div className={`bg-neutral-800 p-6 rounded-md shadow-md ${bookingDrinksList && bookingDrinksList.length > 0 ? 'lg:w-2/3' : 'lg:w-2/3'}`}>
      <div className="mb-6">
  <div className="flex justify-between items-center">
    <button onClick={handleGoBack} className="text-amber-400 flex items-center space-x-2">
      <ArrowBackIcon className="text-amber-400 mr-3" />
      <span>Quay lại</span>
    </button>
    {/* Display time ago, aligned to the right */}
    <span className="text-amber-400 text-neutral-400">{getTimeAgo(bookingData.createAt)}</span>
  </div>
</div>
            ...
            {images.map((image, index) => (
              <div key={index} className="cursor-pointer">
                <div
                  ...
                  style={{
                    paddingBottom: '65%', // 4:3 aspect ratio
                  }}
                >
                  <img
                    src={image}
        ...
        <div className="text-gray-400 text-sm">
          ...
          <p className="mt-4">Giới hạn độ tuổi: <strong>18+</strong></p>
        </div>
      </div>
      ...
      {bookingDrinksList && bookingDrinksList.length > 0 && (
        <div className="lg:w-1/3 space-y-8">
          ...
          <div className="bg-neutral-800 p-6 rounded-lg shadow-lg">
            <h3 className="text-xl text-amber-400 mb-4 text-center">Danh sách thức uống đã đặt trước</h3>
            <div className="space-y-3">
              {bookingDrinksList.map((drink) => (
                <div key={drink.drinkId} className="flex justify-between items-center bg-neutral-700 p-4 rounded-lg shadow-md">
                  <div className="flex items-center space-x-3 w-2/3">
                    ...
                    />
                    <div>
                      <span className="block text-white font-semibold truncate">{drink.drinkName}</span>
                      <span className="block text-amber-400 text-sm">{drink.actualPrice.toLocaleString()} VND</span>
                    </div>
                  </div>
                  <div className="text-right w-1/3">
                    <span className="text-lg font-bold text-white">{drink.quantity}x</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-amber-500 mt-4 pt-4 text-gray-300">
              <div className="flex justify-between">
                ...
                <span className="text-amber-400 font-semibold text-lg">
                  {bookingDrinksList.reduce((total, drink) => total + drink.actualPrice * drink.quantity, 0).toLocaleString()} VND
                </span>
              </div>
            </div>
          </div>
