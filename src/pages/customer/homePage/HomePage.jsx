...
// Tách LocationsList thành component riêng biệt
...
// Tách DrinkCard thành component riêng biệt
const DrinkCard = React.memo(({ imageSrc, name, price }) => (
  <div className="flex flex-col px-2.5 py-3 w-1/6 max-md:w-full transition-transform transform hover:scale-105">
    <div className="flex flex-col grow items-center w-full text-center rounded-xl bg-neutral-700 max-md:mt-7">
      <img loading="lazy" src={imageSrc} alt={name} className="object-contain self-stretch max-h-45 rounded-md aspect-[0.84]" />
...
const BarBuddyDrinks = React.memo(() => {
  const drinkData = useMemo(() => getDrinkData(), []);
