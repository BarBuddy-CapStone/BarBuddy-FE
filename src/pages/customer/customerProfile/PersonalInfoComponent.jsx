...
function PersonalInfo({ accountId }) {
    ...
    const handleFileChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                const response = await AccountService.updateCustomerAvatar(accountId, formData);
                ...
                toast.error("Cập nhật ảnh đại diện thất bại!", { position: "top-right" });
            } finally {
        ...
        <section className="flex flex-col px-8 py-8 mx-auto w-full rounded-md bg-neutral-800 shadow-md max-md:px-5 max-md:mt-10 max-md:max-w-full mr-6">
            ...
            <form className="mt-8 space-y-6">
                <div>
                    ...
                    <input
                        id="fullName"
                        type="text"
                        className={`w-full px-4 py-3 bg-neutral-700 border border-gray-700 rounded-md text-gray-200 text-sm ${isEditing ? '' : 'cursor-not-allowed'}`}
                        value={formData.fullName}
                        onChange={handleChange}
                        readOnly={!isEditing}
                    />
                    {validationErrors.Fullname && (
                        <p className="text-red-500 text-sm">{validationErrors.Fullname.join(', ')}</p>
                    )}
                </div>
                <div>
                    <label htmlFor="birthDate" className="block text-xs text-gray-400 mb-1">
                        Ngày sinh
                    </label>
                    <input
                        id="birthDate"
                    ...
                    {validationErrors.Dob && (
                        <p className="text-red-500 text-sm">{validationErrors.Dob.join(', ')}</p>
                    )}
                </div>
                <div>
                    ...
                    {validationErrors.Phone && (
                        <p className="text-red-500 text-sm">{validationErrors.Phone.join(', ')}</p>
                    )}
                </div>
                <div>
            ...
            {isEditing ? (
                <div className="flex justify-end space-x-4 mt-6">
                    <button className="px-4 py-2 bg-gray-500 text-white rounded-full hover:bg-gray-600" onClick={handleCancel}>
                        Hủy
                    </button>
                    <button className="px-4 py-2 bg-amber-400 text-black rounded-full hover:bg-amber-500" onClick={handleSave}>
                        Lưu
                    </button>
                </div>
            ) : (
                <button className="mt-6 px-4 py-2 bg-amber-400 text-black rounded-full self-end hover:bg-amber-500" onClick={() => setIsEditing(true)}>
                    Chỉnh sửa thông tin
                </button>
            )}
        </section>
